...
let mainHeaderTitle, categorySelect, flashcardElement, wordDisplay,
...
let tagSuggestions = [];
// currentDatasetSource luôn là 'user' vì đã bỏ thẻ của Web
let currentDatasetSource = 'user'; 
window.currentData = [];
...
// Removed sampleData as it's no longer used for web cards
// const sampleData = { ... };
...
const defaultAppState = {
    ...
    // lastSelectedSource: 'web', // Removed as source is always 'user'
...
// getCardIdentifier is no longer needed for web cards, but can be simplified if still used for identifying user cards
// However, user cards already have a unique 'id' from Firestore, so this function can be removed.
    ...
    try {
        ...
        if (s) {
            ...
            appState = {
                ...
                // lastSelectedSource: p.lastSelectedSource || defaultAppState.lastSelectedSource, // Removed
...
async function saveAppState(){
    ...
    // Key for category state no longer includes source as it's always 'user'
    ...
    // Always save deckId as it's always 'user' source now
    ...
    // appState.lastSelectedSource = currentDatasetSource; // Removed
    appState.lastSelectedDeckId = userDeckSelect.value; // Always user deck
...
// Simplified getCategoryState as source is always 'user'
function getCategoryState(src, cat) {
    const key = `user_${cat}`; // Always use 'user_' prefix
...
async function navigateToRecentCard(recentCardInfo) {
    ...
    if (window.currentData && window.currentData.length > 0) {
        foundIndex = window.currentData.findIndex(card => {
            // For user cards, use card.id directly
...
function addCardToRecentlyViewed(cardItem) {
    ...
    // For user cards, use card.id directly as identifier
    const identifier = cardItem.id; 
    ...
    const recentEntry = {
        ...
        // source: currentDatasetSource, // Removed source
        deckId: cardItem.deckId || null
    };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // cardSourceSelect = document.getElementById('card-source-select'); // Removed
    ...
    // copyToDeckModal = document.getElementById('copy-to-deck-modal'); // Removed
    // closeCopyToDeckModalBtn = document.getElementById('close-copy-to-deck-modal-btn'); // Removed
    // copyToDeckSelect = document.getElementById('copy-to-deck-select'); // Removed
    // copyNewDeckNameContainer = document.getElementById('copy-new-deck-name-container'); // Removed
    // copyNewDeckNameInput = document.getElementById('copy-new-deck-name-input'); // Removed
    // copyNewDeckError = document.getElementById('copy-new-deck-error'); // Removed
    // copyToDeckErrorMessage = document.getElementById('copy-to-deck-error-message'); // Removed
    // copyToDeckSuccessMessage = document.getElementById('copy-to-deck-success-message'); // Removed
    // cancelCopyToDeckBtn = document.getElementById('cancel-copy-to-deck-btn'); // Removed
    // confirmCopyToDeckBtn = document.getElementById('confirm-copy-to-deck-btn'); // Removed
    ...
    function populateDeckSelects() {
        // Removed copyToDeckSelect
        const deckSelects = [userDeckSelect, cardDeckAssignmentSelect, jsonCardDeckAssignmentSelect]; 
        deckSelects.forEach(selectEl => {
            if (selectEl) {
                ...
                // Removed "_create_new_deck_" option from copyToDeckSelect
    ...
    function updateSidebarFilterVisibility (){
        ...
        // Always assume 'user' source
        const isUserSource = true; 
        const userId = getCurrentUserId();
    ...
    function updateMainHeaderTitle() {
        const userId = getCurrentUserId();
        // Source text is always "Thẻ của Tôi"
        const sourceText = userId ? "Thẻ của Tôi" : "Thẻ của Tôi (Chưa đăng nhập)"; 
        let deckText = "";
        // currentDatasetSource is always 'user'
        if (userId && userDeckSelect.value && userDeckSelect.value !== 'all_user_cards' && userDeckSelect.value !== 'unassigned_cards') {
        ...
        } else if (userId && userDeckSelect.value === 'unassigned_cards') {
    ...
    async function openAddEditModal(mode = 'add', cardData = null) {
        ...
        // Always show deck assignment container as source is always 'user'
    ...
    // getCardStatus is no longer needed as there are no web cards to get status for.
    // function getCardStatus(cardItem){ 
    //     if (!cardItem) return {};
    //     const userId = getCurrentUserId();
    ...
    async function handleSaveCard() {
        ...
        // Always assign deckId as source is always 'user'
    ...
    async function applyAllFilters(fromLoad=false){
        ...
        // Source is always 'user'
        const sFCSC = getCategoryState('user', cCV); 
        ...
        if (!fromLoad) {
            ...
            // Always set deckId as source is always 'user'
        ...
        // Always filter by user deck if applicable
        ...
        // Always filter by category for user cards
    ...
    async function loadVocabularyData (category) {
        ...
        // Source is always 'user'
        const stateForCurrentSourceCategory = getCategoryState('user', category); 
        ...
        // Filter relevant cards for filters is always activeMasterList as it's all user cards now
        const relevantCardsForFilters = activeMasterList.filter(card => card.category === category); 
    ...
    function updateFlashcard() {
        ...
        if (!item) {
            if(wordDisplay) {
                ...
                } else if ((!userDecks || userDecks.length === 0) && activeMasterList.length === 0) {
                ...
                } else if (activeMasterList.length === 0) { // Removed currentDatasetSource === 'user' && userId
            ...
            // cardOptionsMenuBtn is always visible for user cards
            const canShowOptionsMenu = !!userId; // Simplified, always true if logged in
                ...
                item.meanings.forEach((mObj, idx) => {
                    ...
                    if (mObj.examples && mObj.examples.length > 0) {
                        ...
                        mObj.examples.forEach((ex, exIdx) => {
                            ...
                            copyBtn.onclick = (e) => {
                                e.stopPropagation();
                                document.execCommand('copy'); // Use document.execCommand for clipboard access in iframe
                                showToast('Đã sao chép!', 1500, 'success');
    ...
    function switchToInputMode(mode) {
        ...
        if (mode === 'json') {
            ...
            document.getElementById('json-deck-assignment-container').style.display = 'block';
            modalDeckAssignmentContainer.style.display = 'none'; // Always hide manual deck assignment
            ...
            // Always show manual deck assignment
            modalDeckAssignmentContainer.style.display = 'block'; 
    ...
    async function processAndSaveJsonCards() {
        ...
        for (const cardJson of cardsToProcess) {
            ...
            const cardDataToSave = {
                ...
                deckId: selectedDeckId, // Always assign deckId
                isUserCard: true,
    ...
    // Removed openCopyToDeckModal, closeCopyToDeckModal, handleConfirmCopyToDeck as the Copy to Deck modal is removed.
    ...
    async function openBottomSheet(cardItem, viewType = 'default', subView = null) { 
        ...
        if (viewType === 'default') {
            ...
            // Removed "Copy to My Cards" button as there are no web cards to copy
            ...
            //     hasActions = true;
            // }
            // Always check for user card and logged in
            if (cardItem.isUserCard && loggedInUserId) { 
            ...
            if (youtubeContentDiv) {
                ...
                if (cardItem.videoUrl) {
                    ...
                    if (videoId) {
                        ...
                        iframe.src = `https://www.youtube.com/embed/${videoId}`; // Corrected YouTube embed URL
    ...
    function extractYouTubeVideoId(url) {
        if (!url) return null;
        // Updated regex to correctly extract YouTube video ID
    ...
    function setupEventListeners() {
        ...
        // cardSourceSelect is removed, so this event listener is removed.
    ...
    async function setupInitialCategoryAndSource() {
        ...
        currentDatasetSource = initialSource; 
        // if(cardSourceSelect) cardSourceSelect.value = currentDatasetSource; // Removed
